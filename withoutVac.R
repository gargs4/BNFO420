...
ggp<-ggplot(output, aes(x = time, y = value, color = variable, group = variable))+geom_line(size = 1.3) +
  xlab("Time(days)")+ ylab("Number Susceptible/Infected/Recovered")+ ggtitle("SIRS Model of COVID-19 Spread in Delhi Without Vaccinations")+
...
ggp + ylim(1,6000000) + 
  geom_hline(yintercept = 5244374.6517, linetype = "dotted", color = "magenta")
